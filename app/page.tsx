...
export default function Home() {
  ...
  // ---- Load Firestore history when user signs in ----
  ...
  /** Save to Firestore (if signed in) or sessionStorage, then update local state. */
  async function persistHistoryEntry(leads: EnrichedLead[], label: string) {
    if (user) {
      await saveHistoryEntryToFirestore(user.uid, leads, label);
      // Reload from Firestore so IDs are correct
      const fresh = await loadHistoryFromFirestore(user.uid);
      setHistory(fresh);
    } else {
      const entry = saveHistoryEntry(sessionId, leads, label);
  ...
  async function handleManualEnrich() {
    ...
    await persistHistoryEntry([enriched], label);
    // Mirror into console
  ...
  async function handleEnrichAll() {
    ...
    await persistHistoryEntry(allResults, label);
  ...
  async function handleConsoleEnrichRow(rowId: string) {
    ...
    await persistHistoryEntry([result], result.name || row.name || "1 lead");
  }
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100">
              ...
              user ? (
                <div className="flex items-center gap-2">
                  {user.photoURL && (
                    ...
                    <img src={user.photoURL} alt={user.displayName ?? "User"} className="w-7 h-7 rounded-full border border-zinc-200 dark:border-zinc-700" referrerPolicy="no-referrer" />
                  )}
                  <button
                    onClick={() => signOut()}
                    className="text-xs text-zinc-500 dark:text-zinc-400 hover:text-red-500 transition-colors"
                  >
                    Sign out
                  </button>
                </div>
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-colors"
                >
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex gap-6 items-start">
          ...
          <aside className="hidden lg:flex flex-col w-64 shrink-0 sticky top-4 self-start h-fit bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-zinc-100 dark:border-zinc-800">
              <h2 className="text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
                {user ? "Your History" : "Session History"}
              </h2>
              <p className="text-xs text-zinc-400 font-mono mt-0.5 truncate">
                {user ? (user.displayName || user.email || "Signed in") : (sessionId ? `${sessionId.slice(0, 8)}…` : "—")}
              </p>
            </div>
            ...
            {history.length === 0 ? (
              <div className="px-4 py-8 text-center">
                <p className="text-xs text-zinc-400 italic">
                  {user ? "No history yet." : "No searches yet this session."}
                </p>
              </div>
            ) : (
              <>
